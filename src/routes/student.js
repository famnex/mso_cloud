...
router.get('/card', async (req, res) => {
  ...
  // 1. Prüfen, ob der Benutzer noch in der lokalen Datenbank existiert
  ...
  // 2. Live-Prüfung im LDAP
  const ldapStatus = await ldap.isUserActiveInLdap(user.username);
  if (ldapStatus.error) {
    console.warn(`LDAP-Live-Prüfung fehlgeschlagen bei Ausweisabruf: ${ldapStatus.error}. Verwende Fallback.`);
    if (typeof logEvent === 'function') {
