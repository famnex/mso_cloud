...
/* ==========================================================================
   0. Device Fingerprinting & Persistent Device Token
   ========================================================================== */
function getOrCreateDeviceId() {
  let devId = localStorage.getItem('mso_device_id');
  if (!devId) {
    if (window.crypto && window.crypto.randomUUID) {
      devId = 'dev_' + window.crypto.randomUUID().replace(/-/g, '');
    } else {
      devId = 'dev_' + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
    }
    localStorage.setItem('mso_device_id', devId);
  }
  document.cookie = `mso_device_id=${encodeURIComponent(devId)}; max-age=${5 * 365 * 24 * 60 * 60}; path=/; SameSite=Lax`;
  return devId;
}

function getDeviceFingerprint() {
  let cachedFp = sessionStorage.getItem('mso_fingerprint');
  ...
  try {
    ...
    if (ctx) {
      ...
      ctx.fillStyle = '#069';
      ctx.fillText('MSO Cloud Portal 🛡️', 2, 15);
      ctx.fillStyle = 'rgba(102, 204, 0, 0.7)';
      ctx.fillText('MSO Cloud Portal 🛡️', 4, 17);
    }
    ...
    const fpStr = 'fp_' + Math.abs(hash).toString(16) + '_' + parts.length.toString(16);
    sessionStorage.setItem('mso_fingerprint', fpStr);
    document.cookie = `mso_fingerprint=${encodeURIComponent(fpStr)}; max-age=${5 * 365 * 24 * 60 * 60}; path=/; SameSite=Lax`;
    return fpStr;
  } catch(e) {
    return 'fp_fallback';
  }
...
// Globales Fetch mit Device Headers anreichern
...
window.fetch = function(url, options) {
  ...
  if (options.headers instanceof Headers) {
    options.headers.set('X-Device-Id', devId);
    options.headers.set('X-Device-Fingerprint', fp);
  } else {
    options.headers['X-Device-Id'] = devId;
...
async function loadTiles(isSilentHeartbeat = false) {
  ...
  try {
    ...
    const newHash = JSON.stringify(tiles.map(t => ({ id: t.id, title: t.title, is_time_locked: t.is_time_locked, is_untis_locked: t.is_untis_locked, is_sph_locked: t.is_sph_locked, icon: t.icon, allowed_groups: t.allowed_groups })));
    ...
    tiles.forEach(tile => {
      ...
      const isSphLocked = tile.is_sph_locked === 1;
      const isSph = tile.link && (tile.link.includes('login.schulportal.hessen.de') || tile.link.includes('schulportal') || tile.link.includes('lanis'));
      ...
      if (isLocked) {
        ...
        tileCard.onclick = function(e) {
          e.preventDefault();
          openTileStatusInfoModal('sph-locked', tile.title, '', '', '');
          return false;
        };
      } else {
      ...
      if (currentUser && isSph) {
        if (isLocked || isUntisLocked || isSphLocked) {
          keyBtnHtml = `<button class="tile-key-btn disabled" disabled onclick="event.preventDefault(); event.stopPropagation(); return false;" title="Schulportal ist während der Sperre nicht verfügbar"><i class="fa-solid fa-link-slash"></i></button>`;
        } else {
      ...
      } else if (isUntisLocked || isSphLocked) {
  ...
  if (type === 'time-locked') {
    ...
    messageBox.innerHTML = `
      ...
      <div>
        ...
        Eine Anmeldung bei WebUntis ist erst nach dieser automatischen Einrichtung möglich.<br><br>
        <strong>Du musst aktuell nichts weiter tun und kannst diesen Vorgang nicht beschleunigen. Bitte warte einfach ab, bis die Einrichtung abgeschlossen ist.</strong>
      </div>
    `;
  } else if (type === 'sph-locked') {
    // SCHULPORTAL NOCH NICHT EINGERICHTET
    iconWrapper.style.background = 'rgba(245, 158, 11, 0.15)';
    iconWrapper.style.border = '2px solid #f59e0b';
    icon.className = 'fa-solid fa-user-gear fa-2xl';
    icon.style.color = '#f59e0b';
    ...
    messageBox.innerHTML = `
      <div style="margin-bottom: 8px; font-weight: 700; color: var(--warn-color); display: flex; align-items: center; gap: 8px;">
        <i class="fa-solid fa-triangle-exclamation"></i> Schulportal-Zugang wird eingerichtet
      </div>
      <div>
        Das Schulportal muss für deinen Account erst noch eingerichtet werden.<br><br>
        Eine Anmeldung im Schulportal ist erst nach dieser automatischen Einrichtung möglich.<br><br>
        <strong>Du musst aktuell nichts weiter tun und kannst diesen Vorgang nicht beschleunigen. Bitte warte einfach ab, bis die Einrichtung abgeschlossen ist.</strong>
      </div>
    `;
  }
...
let blockedDevicesPage = 1;
const blockedDevicesPerPage = 25;
let blockedDevicesTotalPages = 1;

async function loadBlockedDevices(resetPage = true) {
  const tbody = document.getElementById('blocked-devices-table-body');
  if (!tbody) return;

  if (resetPage) blockedDevicesPage = 1;

  tbody.innerHTML = `
    <tr>
      <td colspan="7" style="text-align:center; padding:30px; color:var(--text-secondary);">
        <i class="fa-solid fa-spinner fa-spin fa-xl" style="color:var(--accent-color); margin-bottom:10px; display:block;"></i>
        Lade gesperrte Geräte & Fingerprints...
      </td>
    </tr>
  `;

  try {
    const search = document.getElementById('blocked-devices-search')?.value.trim() || '';

    const params = new URLSearchParams({
      page: blockedDevicesPage,
      limit: blockedDevicesPerPage,
      search
    });

    const res = await fetch(`api/admin/proxycheck/devices?${params}`);
    if (!res.ok) {
      const errData = await res.json();
      throw new Error(errData.error || 'Fehler beim Laden der Gerätesperrliste');
    }
    const data = await res.json();

    blockedDevicesTotalPages = data.totalPages;

    renderBlockedDevicesTable(data.devices);
    renderBlockedDevicesPagination(data.total, data.page, data.totalPages);
...
function renderBlockedDevicesTable(devices) {
  ...
  if (!devices || devices.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="7" style="text-align:center; padding:30px; color:var(--text-secondary);">
          <i class="fa-solid fa-laptop-medical" style="font-size:1.8rem; opacity:0.5; margin-bottom:10px; display:block;"></i>
          Keine gesperrten Endgeräte gefunden.
        </td>
      </tr>
    `;
    return;
  ...
  tbody.innerHTML = devices.map(d => {
    const deviceIdShort = d.device_id ? escapeHtml(d.device_id.substring(0, 16) + '...') : '<span style="color:var(--text-secondary);">n/a</span>';
    const fpShort = d.fingerprint ? escapeHtml(d.fingerprint.substring(0, 16) + '...') : '<span style="color:var(--text-secondary);">n/a</span>';
    const userBadge = d.username ? `<span class="badge" style="background:rgba(56, 189, 248, 0.15); color:#38bdf8; font-weight:600;"><i class="fa-solid fa-user"></i> ${escapeHtml(d.username)}</span>` : '<span style="color:var(--text-secondary); font-size:0.8rem;">Unbekannt</span>';
    
    const reasonBadgeMap = {
      vpn: '<span class="badge badge-warning"><i class="fa-solid fa-shield-halved"></i> VPN</span>',
      tor: '<span class="badge badge-danger"><i class="fa-solid fa-globe"></i> TOR</span>',
      proxy: '<span class="badge badge-warning"><i class="fa-solid fa-network-wired"></i> Proxy</span>',
      compromised: '<span class="badge badge-danger"><i class="fa-solid fa-bug"></i> Compromised</span>',
      risk: '<span class="badge badge-warning"><i class="fa-solid fa-triangle-exclamation"></i> High Risk</span>'
    };
    const reasonBadge = reasonBadgeMap[d.reason] || `<span class="badge badge-secondary">${escapeHtml(d.reason)}</span>`;
    
    const blockedAt = d.blocked_at ? new Date(d.blocked_at).toLocaleString('de-DE') : '-';

    return `
      <tr>
        <td>
          <code title="${escapeHtml(d.device_id || '')}" style="font-size:0.8rem; background:rgba(0,0,0,0.2); padding:2px 6px; border-radius:4px;">${deviceIdShort}</code>
        </td>
        <td>
          <code title="${escapeHtml(d.fingerprint || '')}" style="font-size:0.8rem; background:rgba(0,0,0,0.2); padding:2px 6px; border-radius:4px;">${fpShort}</code>
        </td>
        <td>${userBadge}</td>
        <td>${reasonBadge}</td>
        <td><small style="font-family:monospace;">${escapeHtml(d.ip || '-')}</small></td>
        <td><small>${blockedAt}</small></td>
        <td style="text-align:right;">
          <button class="btn btn-sm btn-success" onclick="unblockDeviceAction(${d.id})" title="Gerät entsperren" style="padding:3px 8px; font-size:0.75rem;">
...
function renderBlockedDevicesPagination(total, page, totalPages) {
  const infoEl = document.getElementById('blocked-devices-pagination-info');
  const btnsEl = document.getElementById('blocked-devices-pagination-btns');
  ...
  const start = (page - 1) * blockedDevicesPerPage + 1;
  const end = Math.min(page * blockedDevicesPerPage, total);
  infoEl.innerHTML = `Zeige <strong>${start}–${end}</strong> von <strong>${total}</strong> gesperrten Geräten`;

  let html = '';
  html += `<button class="btn btn-sm btn-secondary" onclick="changeBlockedDevicesPage(${page - 1})" ${page <= 1 ? 'disabled' : ''}><i class="fa-solid fa-chevron-left"></i> Zurück</button>`;
  html += ` <span style="font-size:0.85rem; margin:0 8px;">Seite ${page} von ${totalPages}</span> `;
  html += `<button class="btn btn-sm btn-secondary" onclick="changeBlockedDevicesPage(${page + 1})" ${page >= totalPages ? 'disabled' : ''}>Weiter <i class="fa-solid fa-chevron-right"></i></button>`;

  btnsEl.innerHTML = html;
}

function changeBlockedDevicesPage(newPage) {
  if (newPage < 1 || newPage > blockedDevicesTotalPages) return;
  blockedDevicesPage = newPage;
  loadBlockedDevices(false);
}

async function unblockDeviceAction(id) {
  if (!confirm('Möchten Sie dieses Gerät wirklich entsperren?')) return;
  try {
    const res = await fetch(`api/admin/proxycheck/devices/${id}`, { method: 'DELETE' });
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || 'Fehler beim Entsperren');
    }
    showAdminAlert('Gerät wurde erfolgreich entsperrt.', 'success');
    loadBlockedDevices(false);
  } catch (err) {
    showAdminAlert(err.message, 'danger');
  }
  ...
  list.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div style="font-size:0.78rem; color:var(--text-secondary); display:flex; justify-content:space-between; flex-wrap:wrap; gap:8px;">
        ...
        ${item.is_sph_locked === 1 ? '<span style="color:#f59e0b;"><i class="fa-solid fa-user-gear"></i> SPH-Zugangsdaten fehlen</span>' : ''}
      </div>
    `;
...
async function loadStudentProfile() {
  ...
  try {
    ...
    // Status ermitteln: genehmigt/gedruckt/ausgegeben (1132, 1133) -> Genehmigt & Buttons ausblenden.
    // 1130 (kein Bild), 1131 (eingereicht / noch nicht genehmigt geprüft), 1134 (abgelehnt) -> Upload erlaubt.
    const isApproved = ['1132', '1133'].includes(String(profile.card_status_code)) || 
                       profile.card_status === 'Bild genehmigt' || 
                       profile.card_status === 'genehmigt' || 
                       profile.card_status === 'Bild verifiziert' || 
                       profile.card_status === 'Bild gedruckt' || 
                       profile.card_status === 'Bild ausgegeben' || 
                       profile.card_status === 'Ausweis gedruckt' || 
                       profile.card_status === 'Ausweis ausgegeben';
    const uploadAllowed = !isApproved;
